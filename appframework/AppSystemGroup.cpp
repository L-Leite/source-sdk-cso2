...
bool CAppSystemGroup::AddSystems( AppSystemInfo_t *pSystemList )
{
	...
	{
		g_CSO2DocLog.AddMsg( 1, "AddSystem %s %s", pSystemList->m_pInterfaceName, pSystemList->m_pModuleName );
...
{	
	...
	{
		g_CSO2DocLog.AddMsg( 1, "[Error] System init has failed (ErrorStage:%d)", m_nErrorStage );
		return nRetVal;
	}
